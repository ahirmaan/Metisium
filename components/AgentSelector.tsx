...
const AgentSelector: React.FC<AgentSelectorProps> = ({ allAgents, selectedAgents, onSelectionChange }) => {
  ...
  return (
    <div className="relative" ref={wrapperRef}>
      ...
      >
        {selectedAgents.length > 0 && <img src={selectedAgents[0].avatar} className="w-5 h-5 rounded-full" />}
        <span>{getButtonLabel()}</span>
        <ChevronDownIcon className={`transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
      </button>

      {isOpen && (
        <div className="absolute bottom-full mb-2 w-64 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-10">
          <div className="p-2">
